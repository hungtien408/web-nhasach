...
public partial class giupviec_theogio : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            if (!string.IsNullOrEmpty(Request.QueryString["si"]))
            {
                ...
                hdnServiceDetails.Value = progressTitle(dv[0]["ProductName"].ToString()) + "-pci-" + dv2[0]["ProductCategoryID"].ToString() + "-si-" + dv[0]["ProductID"].ToString() + ".aspx";
            }
            else
            {
